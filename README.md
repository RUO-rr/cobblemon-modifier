...
- **招式数据修改**：主界面 **招式数据** 按钮可直接改**招式本身**的威力 / 命中 / PP / 优先级 / 属性，
  基础招式（952 条）与数据包自定义招式（435 条）都能改；招式脚本是"JSON + 内嵌函数"，
  因此用带括号深度识别的文本扫描器，只动最外层字段、函数体原样保留
...
5. 用下拉框切换插件：**种族值修改 / 宝可梦特性修改 / 技能修改 / 宝可梦属性修改**
6. 改完点 **保存修改**，然后 **重进世界** 生效（不用重启游戏）

> 「技能修改」改的是**宝可梦学得到哪些招式**（学习表）。
> 要改**招式本身**的威力、命中、PP、优先级、属性，点右上角的 **招式数据** 按钮。

常见操作：

- **给宝可梦学本来学不会的招式**：技能修改 → 第一页最上面 4 个空槽填 `1:dragondance`（1 级学龙之舞）
- **改招式本身的威力 / 命中**：点右上角 **招式数据** → 搜索招式（如 `flamethrower`、`whitealbum`）
  → 改威力 / 命中 / PP / 优先级 / 属性 → 保存，重进世界生效
  （基础招式 952 条与数据包自定义招式 435 条都能改）
- **改宝可梦的属性**（如把喷火龙从 火/飞 改成 龙/飞）：插件切到 **宝可梦属性修改** →
  主属性 / 副属性都是下拉框（18 种属性）→ 副属性选「（无副属性）」就是单属性 → 保存
  （形态各自有属性，例如喷火龙 Mega-X 是 火/龙，可以单独改）
